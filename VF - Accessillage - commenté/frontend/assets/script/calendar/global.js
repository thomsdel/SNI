...
// Fonction pour formater la date en "aaaa-mm-jj"
export function formatDate(dateObject) {
    const year = dateObject.getFullYear();
    const month = String(dateObject.getMonth() + 1).padStart(2, '0'); // Mois de 0 à 11, donc +1
    const day = String(dateObject.getDate()).padStart(2, '0'); // Jour du mois

    return `${year}-${month}-${day}`;
}

// Fonction pour formater l'heure en "hh"
export function formatHour(dateObject) {
    return String(dateObject.getHours()).padStart(2, '0'); // Heure avec zéro devant si nécessaire
}
